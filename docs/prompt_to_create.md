...
Now please put on your deep-thinking hat to deeply think and carefully and systematically imagine an elegant and georgeos, Apple UX like calendar plus todo list web application built using Next.js 15 and React 19 with a modern UI built using Tailwind CSS, Radix UI components and Lucide icons. please put on your deep-thinking hat to deeply think and thoroughly explore how best to re-imagine such an application, so intuitive that people will immediately love and get hooked on using it. Just create a detailed comprehensive PRD (Project Requirements Specification) in markdown format. In the PRD, give a very detailed description for the application that clearly and completely defines all its functions, include comprehensive list of technology stacks with versions to use, describe the application interaction with users and between modules using a mermaid flowchart, create a detailed schema using Postgresql 16 and Prisma 6 ORM, add a diagram of codebase file structure and hierarchy, a list of important files with a description of each of their function and interfaces, follow by a detailed execution plan for building the codebase. Break down the comprehensive execution plan in logical and independently executable phases, specify the files to create in each phase in step, and include a detailed checklist for each step.
